...
磁盘余量达到 25% 可恢复热派生并允许有界历史扩量；20% 至 25% 保持热派生，但
历史扩量必须逐批核算。低于 20% 暂停历史扩量与宽研究扫描，只保留不可回补实时
流、实时逐笔物化和受控温层迁移；低于 15% 将已封口的较老实时段与小时成品借用
E 盘温层；低于 10% 只保留不可回补实时流并报警。任何写入出现 `ENOSPC` 或 fsync
失败时立即停止该写者，不能继续推进 checkpoint、manifest 或 coverage。

当前 E 盘角色固定为 SSD 冷事实层兼受控温层，不承载**权威且可变的** SQLite、
WAL、跨进程锁、开放段、信号或执行账。温层可以承载由 C 盘 SQLite 在线备份产生、
通过 quick/FK 校验且可随时重建的只读运行快照；这类快照不是控制面真相源，其锁目录
必须位于被 Git 忽略的数据子目录，不能污染冻结代码树。其余温层只接已封口、有散列、
可重放的制品。接入顺序为：

```text
验证卷 GUID、分区 GUID、卷标、NTFS 与根哨兵
-> 生成只读迁移计划和预计峰值
-> 复制静态制品到目标临时路径
-> 双端逐文件 SHA-256、字节数与 Parquet 元数据验证
-> 只增登记冷位置
-> 暂停相关读写者并原子启用逻辑前缀路由
-> full audit、DuckDB 查询、活动 head 与 UI 回归
-> 保留热副本至少一个完整观察周期
-> 单独释放已验证的可重建热 Parquet
```

释放后若冷盘不可用，按 [分析物化与血缘设计](materialization-design.md) 第 8.2 节
用 `restore-hot --from-raw` 由热层 raw 归档重算并以登记散列相等为门禁恢复热 Parquet。

盘间保留目标如下：

| 层 | 默认位置 | 内容 | 目标水位 |
|---|---|---|---|
| 热控制层 | C 盘项目根 | SQLite/WAL、锁、checkpoint、近期成品、book-state/OFL、query cache、信号与执行账 | 保持 C 盘至少 20%，实盘前目标 25% |
| 热批量层 | D 盘项目根 | 三所 L2 raw 的当前 `.open`、封口段与短期回放窗口 | 保留 7 至 14 日；低于 20% 只迁封口旧段，不动 `.open` |
| 温层 | E 盘 `warm/` | 已封口实时段、小时压实、较老活动分区、迁移双副本 | 最多约 250 GiB，并保留逐制品回迁能力 |
| 冷层 | E 盘 `artifacts/` | 历史 raw/archive、不可变 Parquet、旧 attempt、研究面板与回补 | E 盘保留至少 20% 总容量空闲 |

冻结前向 shadow 使用 E 盘
`warm/frozen-forward-runtime-6d70/` 作为可丢弃运行根。每次运行先对 C 盘控制库做在线
快照，再按活动 GMO BTC 逐笔 head 逐制品校验 SHA-256/字节数并复制或复用，最后原子
替换 E 盘只读快照。预测必须同时满足冻结提交、源码树散列和 clean tree；随后只把内容
寻址目标交给独立执行仓 dry-run。执行账、意图和报告仍在 C 盘独立执行仓，不因 E 盘
可用而迁走。E 盘离线、身份不符、预测超过九十分钟或报告出现任一写端点时，本周期
直接失败，不回退到脏主树。

温层借用不改变 logical storage path、`artifact_id`、`market_id` 或
`normalization_version`。E 盘离线时不得把同路径写回 C 盘后继续假装同一活动世代；
先暂停依赖该输入的派生和新信号，再按迁移台账回迁或重新路由。

容量规划采用范围而不是单点承诺。当前最小常驻范围（三所 BTC/JPY L2、三所 BTC
逐笔及少量 GMO 追加品种）按近期现场估算约 `2.2–3.0 GiB/日`，即约
`0.8–1.1 TiB/年`；若加入 OKX live 400 档，再增加约 `0.5–1.1 GiB/日`。E 盘在
保留 20% 总容量、并将温层限制在约 250 GiB 后，可用于冷事实的预算约 1.2 TiB；因此
最小范围约覆盖 1.1 至 1.5 年，加入 OKX live 后约 0.8 至 1.2 年。每新增市场必须先做
至少二十四小时有界样本，以实测 raw、Parquet、manifest 和压实后字节率替换估算。

D 盘低于 20% 时先停止历史扩量；仍有绝对空间时不为追求百分比而迁移当前 run。需要
借用 E 盘时，只处理已封口且早于保留窗口的完整 run：先复制到 E 盘温层、逐文件复算
manifest/SHA、登记精确 run 前缀路由并观察一个完整物化周期，最后才释放 D 盘副本。
`.open`、checkpoint、当前 run 和无法重放的未封口字节永不跨盘搬移。低于 15% 加速
该封口迁移；低于 10% 进入 raw-only 与报警状态。

首批冻结集合为 OKX 历史 L2 schema v2，活动输出约 14.38 GiB；第二批为历史
`trade-normalization-v1`，活动输出约 7.40 GiB。按 2026-08-22 的冷盘 512 MiB
顺序探针，fsync 写约 268 MiB/s、逐字节散列读约 840 MiB/s；理论纯复制小于两分钟，
运维预算仍按每批十至三十分钟计入小文件、SQLite 冻结、Parquet 元数据和 full audit。
切换前按来源大小另留百分之十工作空间，不能以峰值吞吐替代逐文件验收。

热层恢复顺序固定为 GMO L2、bitbank L2、bitFlyer L2、L2 quality、book-state、
...
约翻倍并触发预测年龄门失败）。持生产写锁的定时任务须共用一张时刻表：
