...
| 编号 | 未决问题 | 备注 |
|---|---|---|
| **TBD-07** | 定时全量对账的周期 | 【提案 2026-08-16，待人工确认】REST 快照 30 秒、超时查询退避 5 至 300 秒、不交易带 0.01，数值与 R-04 余量计算见 [执行链设计](execution-chain-design.md) 第 10 节；已落为版本化配置（G-06） |
...
| **TBD-10** | 熔断阈值具体数值（R-02 的 N 次、断流秒数、余额异动比例） | 【提案 2026-08-16，待人工确认】连续 3 次、断流 90 秒、资产异动 1% 且下限 30 JPY，数值与理由见 [执行链设计](execution-chain-design.md) 第 6 节；已落为版本化配置（G-06） |
| **TBD-11** | 是否使用 `changeOrder`，还是统一 cancel + replace | 报告第 7 节倾向后者；【提案 2026-08-16】执行链统一 cancel + replace，口径见 [执行链设计](execution-chain-design.md) 第 7 节 |
