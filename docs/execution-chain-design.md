...
`data/execution/live/cost-summary/`，用于校准成本模型假设。GMO 现物
taker 手续费按约定逐笔向上取整到整数日元，被取整的小数部分按品种
逐日合算，每个营业日 06:00 JST 返还整数部分；maker 返还逐笔向下取整，
小数部分同样逐日合算后支付。首两笔实测（2026-09-03，各 0.00002 BTC、
名义约 252 JPY）逐笔扣 1 JPY，次日 06:19 JST 余额回升 1 JPY，与该规则
一致：有效费率仍是名义 5 bp，取整只造成日内现金占用与每品种每日不足
1 JPY 的残差，成本汇总按此折算 `fee_bps_effective`。卖出相对参考价滑点
3.7 bp。
...
集合变化时弹出一次系统通知（见[当日快照](2026-09-13-cross-market-valuation-trip-and-empty-head-recovery.md)第 5 节）。浸泡进程仍限模拟运行，
...
live 执行器按差分折算委托：目标持仓（目标乘预算除参考价）减
READ_ONLY 资产 `available` 持仓得差分，正买负卖，差分名义低于
`no_trade_band` 乘预算不发单（第 9 节口径；2026-09-04 前误用从零
折算，会在目标持续非零时每轮重复买入且目标归零时永不卖出）。执行分两阶段（2026-09-04）：第一阶段以 Post-only（`timeInForce=SOK`，GMO
现物全品种可用）挂在同侧最优价，等待 `passive_wait_seconds`（执行配置，
缺省 150 秒）；成交即结束，被交易所因会立即成交而作废（`EXPIRED_SOK`）或
届满撤单确认后，以剩余数量按对手侧最优价发第二阶段委托，等待剩余窗口。
两阶段是独立意图、同一 `correlation_id`（TBD-11 先撤后下），用量按每笔
意图保守计数。GMO 现物 Maker 返还 0.01%、Taker 收 0.05%，2026-09-04 影子
测量触价成交 60 秒逆向选择约 1.7 bp，被动成交单边成本约 0.7 bp，吃单约
5.5 bp。未配置被动阶段时直接以对手侧最优价单笔委托，买入前
核可用 JPY 不低于名义额乘 1.001，否则记零写跳过（R-06）。启动时拒绝
`--rules`、`--reference-price`、`--book`、`--service-status` 覆盖
（R-03），并要求伴随观察进程心跳不超过五分钟，否则以 `observer_stale`
拒绝。撤单请求异常不崩溃：记写路径异常并继续按 READ_ONLY 轮询终态，
未到终态即在报告中明示委托可能仍挂在交易所。发送超时先等五秒让交易所
落账，零候选再等五秒复查后才判 FAILED。连续写路径异常计数随信封状态
持久化并在下一周期注入熔断器（R-02）。用量文件缺行时从账本已消耗写
预算的意图补记（T-11）。交易所接口错误、网络超时与目标校验失败一律
转为拒绝报告（维护窗记 `maintenance`），不再以崩溃退出。
