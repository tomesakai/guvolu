...
## 4. XRP 与 SOL 预注册复现（2026-09-04 补记）

同一流程在 GMO XRP（`research-run-7b3387f6`，面板自 2019-02-01，26 折）与
GMO SOL（`research-run-542c2b27`，面板自 2023-09-01，仅 7 折、15,120 根样本外柱）
上重复，配置为 `strategy_research_xrp_gmo_replication.json` 与
`strategy_research_sol_gmo_replication.json`，均为决策级：

| 指标 | XRP price_breakout | XRP trend | SOL price_breakout | SOL trend |
|---|---|---|---|---|
| 拼接 OOS Sharpe | 0.733 | 1.043 | 0.259 | 0.704 |
| 最大回撤 | 0.553 | 0.452 | 0.257 | 0.211 |
| FDR q | 0.069 | 0.024 | 0.777 | 0.777 |
| bootstrap p | 0.051 | 0.005 | 0.336 | 0.178 |
| DSR（有效计数） | 0.951 | 0.989 | 0.599 | 0.780 |
| 固定多头基准 Sharpe | 0.279 | 0.279 | -0.384 | -0.384 |
| 基准超额 | 0.453 | 0.763 | 0.643 | 1.088 |
| 正收益折比 | 0.38 | 0.58 | 0.29 | 0.57 |
| 折冠军众数 | 168 / 0.4 | 168、entry 1.0 | 72 / 0.4 | 168 / 0.5 / 0 / 0.4 |
| 裁决 | 回撤超限、正折比与 bootstrap 失败 | 回撤 0.452 超 0.45 上限 | 全面失败 | FDR、bootstrap、DSR 失败 |

四品种合读：trend 家族在 BTC、ETH、XRP 三个长样本上 Sharpe 0.78 至 1.04、
DSR 0.975 至 0.989、基准超额 0.43 至 0.76，XRP 只差回撤上限 0.002；
price_breakout 在 ETH 复现、在 XRP 因回撤与折比失败。SOL 样本只有三年且
买入持有为负，两家族都远未达显著，只能作为方向一致的弱证据。SOL 运行曾因
四个零行实时段头共用同一制品而失败（面板逐文件控制合同失配），合并模块
已改为先撤销零行段头。

## 5. 后续

后继冻结计划候选以预注册网格的跨品种一致性为准入维度：trend 168 系是当前
唯一在三个长样本品种上同时通过统计门的候选，price_breakout 保留为 BTC 与
ETH 两品种证据。四个品种同参数的联合准入与回撤口径统一由多品种 GPU 评估
承担。
