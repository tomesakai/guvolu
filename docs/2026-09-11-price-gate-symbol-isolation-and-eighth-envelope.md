...
## 6. 同日补记：第九封与首单 canary

第八封于 09-11 08:11 JST 签发（身份 `d9011cf01af8`）。复核发现首单 canary 门是「超过即拒绝」而非压缩下单（`check_first_order_canary`，合同第 14 节「首单拒超」），而预算 10,000 下的最大首单为 BTC 6,000、ETH 5,800，均高于第八封的 5,000：突破信号触发的首单会被逐小时拒绝，直到目标暴露降到 0.5 以下。第九封只把 `canary_first_order_jpy_max` 改为 6,000（等于两市场最大首单），其余字段与第八封相同。最小手数实盘已由 09-03 两笔成交证明，本封的首单压额不再是最小手数级。

第八封下首两轮（09:12 BTC、09:30 ETH）门禁全部通过、无误暂停。复核状态文件时发现参考价观测的保留窗口是常量 600 秒，而急变门窗口为 3,900 秒：每小时任务下同一品种永远只有一条观测，急变门在单市场上从未真正生效；此前的误暂停也只在 BTC 链运行较长、ETH 观测落在其 10 分钟内时出现，与每日 1 至 3 次的频率一致。修复为 live 执行器按信封 `price_move_pause.window_seconds` 裁剪观测，急变门自此对每小时相邻观测生效。

另记：第八封生效前（`valid_from` 为 00:00Z，即 09:00 JST）的 08:12 与 08:30 两轮被正当拒绝 `envelope_expired`；主仓提交到执行仓快进之间的五小时（03:30 至 07:30 JST）每轮以「risk_budget_jpy 与执行配置不一致」失败，属预期的版本错位窗口，此后不再出现。
