...
成交保守成本。手续费假设按场所名义费率计入，不设逐笔下限：GMO 现物 taker 费按约定逐笔
向上取整到整数日元，但被取整的小数部分按品种逐日合算并于每个营业日 06:00 JST 返还整数
部分（maker 返还逐笔向下取整、同样逐日合算支付），2026-09-03 两笔 252 JPY 名义成交逐笔扣
1 JPY、次日 06:19 JST 返还 1 JPY 与之一致；有效费率等于名义费率，残差每品种每日不足
1 JPY，只影响日内现金占用，不构成小名义单的额外成本。GMO BTC/ETH/XRP/DAI 的 taker
费率为 0.05%，即 `fee_bps_assumption: 5.0`；其余品种 0.09%。网格另由第 8 节的 L2 事件 shadow 给出被动成交上下界；撤单失败、部分成交和
...
冻结快照刷新留出窗口；进入执行适配前的预测年龄硬限为 55 分钟，晚到周期失败关闭
（2026-08-30 经人工确认由 45 分钟两次修订至 55：冻结预测器稳态管线实测 33 至
39 分钟、交互负载下观测最大年龄 50.9 分钟；55 分钟仍落在预测目标柱有效期
六十分钟与治理登记窗口六十五分钟之内。六十分钟是语义边界，本 vintage 内不再
放宽；后继冻结计划须在封存前实测证明管线耗时预算）。

当前 Windows 权限策略拒绝无密码 `S4U` 注册，任务只能在用户登录期间运行；关机或注销
超过两小时会造成不可补算的预测缺口，必须由每日健康巡检报警。任何 E 盘身份/哨兵失败、
冻结树非 clean、输入散列不符、预测年龄超过五十五分钟、dry-run 模式不符或
...
1. 每小时任务退出码为零，决策时点连续推进，进入执行适配时预测年龄不超过五十五分钟；
...
实现提供数值参考。其边界同样明确：全部方向流派的仓位状态机为纯多头（现物无做空路径），
组合在下跌状态只能以空仓防守，杠杆做空分支在平仓权限开通前类型级不可达（T-09）；
当前只有单市场小时面板；参数网格是人工提出的；非正态
...
side 的复现是同价同量的多笔撮合，官方归档同样存在，只登记不约束。tile 小时没有绑定任何逐笔件（逐笔断流覆盖整小时，如 GMO 2026-08-22T00:00Z 至 02:10Z）属合法输入，该小时的桶降级为不可信而不中止运行。GMO 2026-08-22 切换
