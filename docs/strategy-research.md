...
冻结快照刷新留出窗口；进入执行适配前的预测年龄硬限为 50 分钟，晚到周期失败关闭
（2026-08-30 经人工确认由 45 分钟修订：冻结预测器稳态耗时实测使 45 分钟无余量，
50 分钟仍落在预测目标柱有效期六十分钟与治理登记窗口六十五分钟之内）。

当前 Windows 权限策略拒绝无密码 `S4U` 注册，任务只能在用户登录期间运行；关机或注销
超过两小时会造成不可补算的预测缺口，必须由每日健康巡检报警。任何 E 盘身份/哨兵失败、
冻结树非 clean、输入散列不符、预测年龄超过五十分钟、dry-run 模式不符或
...
1. 每小时任务退出码为零，决策时点连续推进，进入执行适配时预测年龄不超过五十分钟；
